...
function RollingPlaceholder() {
  ...
  return (
    <span aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 flex items-center overflow-hidden px-2">
      <span key={index} className="rolling-prompt block w-full truncate text-sm leading-5">{PROMPTS[index]}</span>
    </span>
  )
}
  ...
  : revealing
    ...
    : message.plaintext}</div>
          ...
          <form className="chat-composer relative p-4 sm:p-5" onSubmit={send}>
            {stickerPickerOpen && (
              <div onClick={(event) => event.stopPropagation()} className="sticker-pop absolute bottom-[5.4rem] left-4 z-30 w-[min(26rem,calc(100vw-2rem))] rounded-[2px] border border-[#ffd100]/70 bg-[#12010a] p-3 shadow-[8px_8px_0_rgba(0,0,0,.5)] sm:left-5">
                <div className="mb-3 border-b border-[#ffd100]/25 px-1 pb-2"><p className="text-sm font-bold text-[#ffd100]">Class stickers</p><p className="mt-0.5 text-[10px] text-[#fff6dc]/50">Pick one or type its :shortcode:</p></div>
                <div className="grid max-h-80 grid-cols-2 gap-2 overflow-y-auto pr-1 sm:grid-cols-3">
                  {STICKERS.map((sticker) => <button type="button" key={sticker.name} onClick={() => { setDraft((current) => `${stickerShortcode(sticker.name)}${current.trim() ? ` ${current}` : ''}`); setStickerPickerOpen(false) }} className="group overflow-hidden rounded-[2px] border border-[#ffd100]/25 bg-[#1c0309] p-1.5 text-left transition hover:-translate-y-0.5 hover:border-[#ffd100] hover:bg-[#2a0510]"><StickerImage sticker={sticker} compact /><span className="mt-1.5 block truncate px-1 pb-0.5 font-mono text-[10px] text-[#fff6dc]/55 group-hover:text-[#ffd100]">:{sticker.name}:</span></button>)}
                </div>
              </div>
            )}
            {emojiPickerOpen && (
