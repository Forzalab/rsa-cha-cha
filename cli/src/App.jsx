  ...
  return (
    <main onClick={() => { setOpenReactions(null); setStickerPickerOpen(false); setEmojiPickerOpen(false) }} className="aurora grid-glow relative min-h-screen overflow-hidden p-3 sm:p-7">
      ...
      <section className="chat-shell relative z-10 mx-auto flex h-[calc(100vh-1.5rem)] max-w-6xl overflow-hidden rounded-[1.75rem] border border-slate-300/10 shadow-[0_30px_100px_rgba(0,0,0,.55)] backdrop-blur-xl sm:h-[calc(100vh-3.5rem)]">
        <div className="pointer-events-none absolute inset-x-24 top-0 h-px bg-gradient-to-r from-transparent via-amber-300/60 to-transparent" />
        <aside className="chat-sidebar hidden w-64 shrink-0 border-r border-slate-300/8 p-5 md:flex md:flex-col">
          ...
          <div className="mt-auto space-y-3">
            <button type="button" tabIndex={showNewMessages ? 0 : -1} aria-hidden={!showNewMessages} onClick={() => scrollToNewest()} className={`flex w-full items-center justify-center gap-2 rounded-xl border border-amber-200/20 bg-amber-400 px-3 py-2.5 text-xs font-semibold text-amber-950 transition-all duration-300 hover:bg-amber-300 ${showNewMessages ? 'new-message-badge opacity-100' : 'pointer-events-none translate-y-2 opacity-0'}`}><ArrowDown size={14} /> View new messages</button>
            <div className="rounded-2xl border border-amber-300/10 bg-amber-400/[.035] p-4">
              <div className="flex items-center gap-2 text-xs font-medium text-amber-300"><ShieldCheck size={14} /> RSA channel active</div>
        ...
        <div className="chat-pane relative flex min-w-0 flex-1 flex-col">
          ...
          <div ref={messageListRef} onScroll={handleMessageScroll} className="flex flex-1 flex-col gap-5 overflow-y-auto scroll-smooth p-5 sm:p-7">
            {chat.messages.length === 0 && (
              <div className="m-auto max-w-sm text-center"><LockKeyhole className="mx-auto text-slate-700" size={38} /><h3 className="mt-4 font-medium text-slate-300">The room is quiet</h3><p className="mt-2 text-sm leading-6 text-slate-600">Open another browser tab, join with a different name, or have a classmate join then send the first encrypted message!</p></div>
            )}
          ...
          <form className="chat-composer relative border-t border-slate-300/8 p-4 sm:p-5" onSubmit={send}>
            ...
            {emojiPickerOpen && (
              <div onClick={(event) => event.stopPropagation()} className="sticker-pop absolute bottom-[5.4rem] left-4 z-30 w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-amber-200/10 bg-[#1b181f]/95 p-3 shadow-2xl shadow-black/60 backdrop-blur-xl sm:left-5">
                <div className="mb-3 flex items-center justify-between px-1"><div><p className="text-sm font-semibold text-slate-200">Emoji</p><p className="mt-0.5 text-[10px] text-slate-600">Add some personality to your message</p></div><Smile size={17} className="text-amber-300" /></div>
                <div className="grid max-h-64 grid-cols-8 gap-1 overflow-y-auto pr-1">
                  {EMOJIS.map((emoji) => <button type="button" key={emoji} onClick={() => setDraft((current) => `${current}${current && !current.endsWith(' ') ? ' ' : ''}${emoji}`)} className="grid aspect-square place-items-center rounded-lg text-xl transition duration-150 hover:scale-125 hover:bg-amber-400/10" aria-label={`Add ${emoji}`}>{emoji}</button>)}
                </div>
              </div>
            )}
            <div className="flex items-end gap-2 rounded-2xl border border-white/10 bg-black/25 p-2 shadow-inner shadow-black/20 transition-all duration-300 hover:border-white/20 hover:bg-black/30 hover:shadow-[0_0_24px_rgba(52,211,153,.045)] focus-within:border-amber-400/35 focus-within:bg-black/30 focus-within:shadow-[0_0_32px_rgba(52,211,153,.085)]">
              <button type="button" onClick={(event) => { event.stopPropagation(); setStickerPickerOpen((open) => !open); setEmojiPickerOpen(false); setOpenReactions(null) }} disabled={!joined} aria-label="Open image picker" className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border transition disabled:opacity-30 ${stickerPickerOpen ? 'border-amber-300/30 bg-amber-400/10 text-amber-300' : 'border-transparent text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}><ImagePlus size={19} /></button>
              <button type="button" onClick={(event) => { event.stopPropagation(); setEmojiPickerOpen((open) => !open); setStickerPickerOpen(false); setOpenReactions(null) }} disabled={!joined} aria-label="Open emoji picker" className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border transition disabled:opacity-30 ${emojiPickerOpen ? 'border-amber-300/30 bg-amber-400/10 text-amber-300' : 'border-transparent text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}><Smile size={19} /></button>
